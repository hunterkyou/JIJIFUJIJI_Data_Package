...
execute if entity @a[tag=liar_bar_shoot,team=liar_bar_a_table,tag=a_table_last,limit=1,sort=random] as @a[tag=liar_bar_shoot,team=liar_bar_a_table,tag=a_table_last,limit=1,sort=random] run return run tag @s add a_table_playing
#如果存在玩家是开枪 但是没死 并且是上家就运行
execute if entity @a[tag=liar_bar_shoot,team=liar_bar_a_table,limit=1,sort=random] as @a[tag=liar_bar_shoot,team=liar_bar_a_table,limit=1,sort=random] run return run tag @s add a_table_playing
#如果存在玩家是开枪 但是没死 
...
#如果没有tag=liar_bar_shoot存活，就选择给a_table_after_gun
...
#玩家死亡了——#liar_bar_shoot 全部死亡，从或者的下家开始设置为a_table_playing

#非恶魔牌
        #没死人        liar_bar_shoot
        #死人了          a_table_playing的下一个出
#恶魔牌
        #没死人或者没死完        liar_bar_shoot 选一个    a_table_last优先
